...
local MaxMana = 2400 --- How many mana potions you begin the hunt with.
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	elseif (labelName == "CheckSoft") then  
		...
		if ((SoftBoots == true) and (Self.ItemCount(6530) > 1)) then
			print([[3x SOFTY ZUZYTE BIEGNE ZROBIC REFILL < 3..]])
	...
	elseif (labelName == "MAYHEM CHECK") then  
		Walker.Stop()
		if ((EnchantMayhem == true) and (Self.ItemCount(23228) > 0)) then
			print([[MOJ MAYHEM WYCZERPANY IDE DOLADOWAC..]])
			Walker.Goto("GO ENCHANT")
		else		
print([[MAYHEM GOTOWY]])
			Walker.Goto("MAYHEM DONE")
		end
		wait(1500,2000)
		Walker.Start()	
...
elseif (labelName == "Mayhem") then  
		Walker.Stop()
		Creature.Follow("Yana")
		wait(1500,2000)
		Self.SayToNpc("hi")
		wait(1500,2000)
		Self.SayToNpc("token")
		...
		Self.SayToNpc("yes")
		wait(1500,2000)
		Self.SayToNpc("charge")
		wait(1500,2000)
		Self.SayToNpc("yes")
		wait(1500,2000)
		Walker.Start()	

elseif label == "Mayhem" then
setWalkerEnabled(false)
Self.SayToNpc({'hi','token','gold','yes','charge','yes'},65)
wait(200, 500)
setWalkerEnabled(true)	
...
elseif (labelName == "Wait") then
    ...
    print([[Withdraw Rings]])
     ...
     if (Self.ItemCount(RingID) < MaxRings) then
            Self.WithdrawItems(20, {RingID, MaxRings, (MaxRings-Self.ItemCount(RingID, 0))})
            wait(1500,1900)
    end
		else
	        Walker.Goto("NoRings")
...
Walker.Stop()
	...
	elseif (labelName == "TransferMoney") then  
		Walker.Stop()
		wait(1500,2000)
		Self.SayToNpc("hi")
		wait(1500,2000)
