...
local MaxHealth = 1100 --- How many health potions you begin the hunt with.
...
local info = [[
~~~~UNDEADS ORAMOND BY Gucci NEW 2017 < 3 PRIVATE SCRIPT~~~~
]]
    print([[
~~~~UNDEADS ORAMOND BY Gucci NEW 2017 < 3 PRIVATE SCRIPT~~~~]])
    wait(30000)
...
{5884, 0}, 	-- stone skin amulet
  ...
  ----------------------------------------------------------------------------------------------------
     elseif (labelName == "Check") then
       	setWalkerEnabled(false)	
		if (Self.Cap() > MinCap and Self.ItemCount(HealthID) > MinHealth) and (Self.ItemCount(ArrowID) > MinArrow) and (Self.Stamina() > 840) then
				print("Hunt Again..:")
							setWalkerEnabled(true) 
			gotoLabel("Hunt")
		else
