...
local MinAvalanche = 40 --- How many health potions until you leave the hunt.
local MaxAvalanche = 1200 --- How many health potions you begin the hunt with.
local AvalanchePrice = 45 --- What is the price of your selected mana potions?

local SoftBoots = false
--CAP--
...
local PotionBp  = "buggy backpack" ---- Potion Bp
...
Self.ReachDepot = function (tries)
	...
	local function gotoDepot()
		...
		print("Znalazlem depo chujku " .. tostring(#DepotPos))
		for i = 1, #DepotPos do
			location = DepotPos[i]
			...
			if Self.DistanceFromPosition(pos.x, pos.y, pos.z) >= 1 then
				...
				print("Jakis frajerek blokuje dostep do dp.")
			end
		end
		return false
	end
...
---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
	elseif (labelName == "CheckSoft") then  
		...
		if ((SoftBoots == true) and (Self.ItemCount(6530) > 0)) then
			print([[Worn Soft boots =2 1, ide naladowac sofciki.. HA]])
...
elseif (labelName == "CheckSoft") then  
		Walker.Stop()
		if ((SoftBoots == true) and (Self.ItemCount(6530) > 0)) then
			print([[Worn Soft boots =2 1, lets go refill.. HA]])
			Walker.Goto("RefillSoft")
		else
			print([[Skip softs.]])
			Walker.Goto("DoneSoft")
 ...
 ----------------------------------------------------------------------------------------------------
     elseif (labelName == "Check") then
		...
		if (Self.ItemCount(ManaID) > MinMana) and (Self.Stamina() > 720) and (Self.ItemCount(HealthID) > MinHealth) and (Self.ItemCount(AvalancheID) > MinAvalanche)then
				print("Starczy mnie supli lecem dalej")
							setWalkerEnabled(true) 
			gotoLabel("Hunt")
		else
			gotoLabel("Refill")	
				print("malo manachow lecem refil ha")
			setWalkerEnabled(true) 
				end------------------------------------------------------------------------
...
----------------------------------------------------------------------------------------------------
	 elseif (labelName == "Check1") then
       	setWalkerEnabled(false)	
		if (Self.ItemCount(ManaID) > MinMana) and (Self.Stamina() > 720) and (Self.ItemCount(HealthID) > MinHealth) and (Self.ItemCount(AvalancheID) > MinAvalanche)then
				print("**Expienie Wznowione HA!**")
							setWalkerEnabled(true) 
		else
			gotoLabel("Refill")	
				print("**Wracam Do Miasta HA!**")
			setWalkerEnabled(true) 
				end
----------------------------------------------------------------------------------------------------
